...
public class SelectDestinationToEnemiesNode : Node
{
    ...
    private bool CanBuff(int _pa, BlackBoard _bb)
    {
        ...
        if (_bb.Character.CurrentPa - _pa > _bb.SpellHolder.Cost)
        {
            ...
            {
                if (Vector3.Distance(_bb.Character.transform.position, _bb.Targets[i].transform.position) < _bb.SpellHolder.Spell.Range)
                {
                    valid++;
                }
            }
