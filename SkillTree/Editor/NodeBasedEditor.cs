...
public class NodeBasedEditor : EditorWindow
{
    ...
    private void OnClickAddNode(Vector2 mousePosition)
    {
        ...
        nodes.Add(new NodeTree(mousePosition, 400, 150, nodeStyle, selectedNodeStyle,
            inPointStyle, outPointStyle, OnClickInPoint, OnClickOutPoint, OnClickRemoveNode,
            nodeCount, false, 0, null, 0, null));
        ++nodeCount;
    }
    ...
    private void OnClickRemoveNode(NodeTree node)
    {
        if (connections != null)
        {
    ...
    private void SaveSkillTree()
    {
        if (nodes.Count > 0)
        {
            ...
            for (int i = 0; i < nodes.Count; ++i)
            {
                ...
                skillTree.skilltree[i].skill_DependenciesUnlocked = dependencies;
            }
        ...
        if (File.Exists(path))
        {
            ...
            if (File.Exists(path))
            {
                ...
                for (int i = 0; i < _skillTree.Length; ++i)
                {
                    ...
                    if (_skillTree[i].skill_DependenciesUnlocked.Length == 0)
                    {
                ...
                Skill outSkill;
                NodeTree outNode = null;

                // Create connections
                for (int i = 0; i < nodes.Count; ++i)
                {
                    for (int j = 0; j < nodes[i].skill.skill_DependenciesUnlocked.Length; ++j)
                    {
                        if (skillDictionary.TryGetValue(nodes[i].skill.skill_DependenciesUnlocked[j], out outSkill))
                        {
                            for (int k = 0; k < nodes.Count; ++k)
                            {
    ...
    private void LoadSkillCreateNode(Skill skill, Vector2 position)
    {
        ...
        nodes.Add(new NodeTree(position, 400, 150, nodeStyle, selectedNodeStyle,
            inPointStyle, outPointStyle, OnClickInPoint, OnClickOutPoint, OnClickRemoveNode, 
            skill.id_Skill, skill.unlocked, skill.cost, skill.skill_DependenciesUnlocked, skill.spellID, skill.alterationName));
