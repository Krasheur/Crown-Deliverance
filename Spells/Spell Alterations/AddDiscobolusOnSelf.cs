...
public class AddDiscobolusOnSelf : AddAlterationOnSelf
{
    public override void OnFireProjectile(Projectile _projectile)
    {
        if (_projectile.Emitter)
        {
            Discobolus alt = Instantiate(alteration, _projectile.Emitter.transform) as Discobolus;
            alt.Emitter = _projectile.Emitter;
            alt.Projectile = _projectile;
        }
    }
